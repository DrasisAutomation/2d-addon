...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const LOAD_PAGE_FILES = ["load1.json", "load2.json", "load3.json"]
    ...
    function loadDesignFromURL(url) {
        currentLoadPage = getPageFromFileName(url)
        setActiveNavPage(currentLoadPage)
...
function updateFooter(text) {
    const connectionStatus = CONNECTION_MODE === 'local' ? '🔒 Local' : '🌐 Remote';
    const footer = document.querySelector(".footer");

    if (!footer) return;

    // Store the current text before rebuilding
    const currentText = text || 'Ready';

    const navItems = LOAD_PAGE_FILES.map((file, index) => {
        const page = index + 1
        const activeClass = page === currentLoadPage ? 'active' : ''
        return `
              <a href="#" class="nav-square ${activeClass}" data-load-file="${file}" data-page="${page}">${page}</a>`
    }).join('')
      ...
      <div style="display: flex; align-items: center; gap: 10px; width: 100%; justify-content: space-between;">
        <div style="display: flex; align-items: center; gap: 10px;">
          ...
          <div class="nav-container" style="display: flex;">
            <div class="nav-scroll">${navItems}
            </div>
          </div>
        </div>
        
        <div style="display: flex; gap: 10px; align-items: center;">
          <span style="font-size: 12px; color: #ccc;">${currentText}</span>
          <button class="footer-control reset-footer-btn" id="footerResetBtn"
          style="background: #666; padding: 4px 8px; border-radius: 4px;">
          ↻
          </button>
          <button class="header-control switch-btn" id="switchBtn" title="Switch Connection">
          <i class="fas fa-exchange-alt"></i>
          </button>
          <button class="footer-control load-btn" id="loadBtn">📂</button>
        </div>
      </div>
    `;

    // IMPORTANT: Re-attach event listeners to the NEW elements
    reattachFooterListeners();
}

// Add this function to reattach event listeners
function reattachFooterListeners() {
    const footerResetBtn = document.getElementById("footerResetBtn");
    if (footerResetBtn) {
        // Remove existing listeners first to prevent duplicates
        footerResetBtn.replaceWith(footerResetBtn.cloneNode(true));
        const newResetBtn = document.getElementById("footerResetBtn");
    ...
    if (newLoadBtn) {
        ...
        newLoadBtn.replaceWith(newLoadBtn.cloneNode(true));
        const freshLoadBtn = document.getElementById("loadBtn");
        
        freshLoadBtn.addEventListener("click", () => {
            loadDesignFromURL(DEFAULT_LOAD_FILE); // This should directly load load.json
        });
    }
    ...
    if (switchBtn) {
        ...
        const freshSwitchBtn = document.getElementById("switchBtn");
        
        freshSwitchBtn.addEventListener("click", handleSwitchConnection);
    }

    const navButtons = document.querySelectorAll(".nav-square[data-load-file]");
    navButtons.forEach((navButton) => {
        navButton.addEventListener("click", (event) => {
            event.preventDefault()
            const targetFile = navButton.dataset.loadFile
            const targetPage = Number(navButton.dataset.page) || 1

            if (!targetFile) return

            currentLoadPage = targetPage
            setActiveNavPage(currentLoadPage)
            loadDesignFromURL(targetFile)
        })
    })
}
...
function setActiveNavPage(page) {
    ...
    document.querySelectorAll(".nav-square[data-load-file]").forEach((button) => {
        const buttonPage = Number(button.dataset.page) || 0
        button.classList.toggle("active", buttonPage === page)
    })
}

function getPageFromFileName(fileName) {
    const match = /load(\d+)\.json$/i.exec(fileName)
    return match ? Number(match[1]) : currentLoadPage
}

    // Add this handler function for switching connections
    function handleSwitchConnection() {
        if (confirm("Switch connection mode? You'll need to reconnect.")) {
    ...
    function init() {
        ...
        if (initialSwitchBtn) {
            initialSwitchBtn.addEventListener("click", handleSwitchConnection);
        }
